#for each phone, associate the phone with a word and grammatical information, possibly by changing time values for flex imported word annotations``

#ANNOTATION_REF equals the ANNOTATION_ID of parent tier annotation

##PREVIOUS_ANNOTATION lists ANNOTATION_ID of previous annotation
library(stringi)
library(devtools)
library(dplyr)
#install_github("dalejbarr/elan") #need to install Rtools on system
library(elan)
library(rPraat)
library(audio)
library(tuneR)
library(stringr)
library(seewave)
library(ggplot2)
library(ggpubr)

{
  index_offset <- 1184
  #set praat_table_file name
  #praat_table_file <- "eibela_data_20200828.csv"
  praat_table_file <- "praat_table_updated_last_segnum_54_index_36605.csv"
  
  #sets the name of the folder containing eaf files
  languageFolder <- './rawinput'
  MC_file_list <- c("ailoa0505-3", "ailot0319-3_Final", "ailot0317-3_Final")
  #fix wese files word classes, add graid, refind, isnref, add IDs, add 
  #fix MC_phone count counting, fix 505 segment ~212 that necessitated +- 30 ms buffer
  #[1] "word:  oːlaːbɪ segmented_morphemes:  oː+laː+-bɪ word_POS  word_gloss:  oh+REPORTED?+D.S segment_number:  110"
  #[1] "Index: 37437"
  #[1] "word:  oːlaːbɪ segmented_morphemes:  oː+laː+-bɪ word_POS  word_gloss:  oh+REPORTED?+D.S segment_number:  110"
  #[1] "Index: 37438"
  #[1] "word:  oːlaːbɪ segmented_morphemes:  oː+laː+-bɪ word_POS  word_gloss:  oh+REPORTED?+D.S segment_number:  110"
  #[1] "Index: 37439"
  #[1] "word:  ăkaː segmented_morphemes:  oː+laː+-bɪ word_POS  word_gloss:  oh+REPORTED?+D.S segment_number:  110"
  #[1] "Index: 37440"
  #[1] "word:  ăkaː segmented_morphemes:  oː+laː+-bɪ word_POS  word_gloss:  oh+REPORTED?+D.S segment_number:  110"
  #[1] "Index: 37441"
  #[1] "word:  ăkaː segmented_morphemes:  oː+laː+-bɪ word_POS  word_gloss:  oh+REPORTED?+D.S segment_number:  110"
  #[1] "Index: 37442"
  


  
  #set tier names
  segnum_name <- "phrase-segnum-en"
  flex_word_tier_name <- "word-txt"
  flex_morph_tier_name <- "morph-txt"
  flex_gloss_tier_name <- "morph-gls"
  word_class_name <- "morph-msa-en"
  phone_tier_name <- "transcription - phones"
  phone_word_name <- "transcription - words"
  
  #Multi-CAST tiernames
  MC_segnum_name <- "utterance_id"
  MC_utterance_name <- "utterance"
  MC_word_tier_name <- "grammatical_words"
  MC_gloss_tier_name <- "gloss"
  MC_graid_tier_name <- "graid"
  MC_refind_tier_name <- "refind"
  MC_isnref_tier_name <- "isnref"
 
  #creates list of eaf files
  fls <- list.files(languageFolder, full.names = F, pattern = 'eaf')
  full_fls <- list.files(languageFolder, full.names = T, pattern = 'eaf')
  MC_files <- full_fls[grepl("-3", full_fls)]
  full_fls <- full_fls[grepl("-2", full_fls)]
  
  #add folder to file name
  praat_table_path <- paste(languageFolder, praat_table_file, sep = "/")
  
  #load praat spreadsheet
  ### Encoding note, when loading and saving table, no not set to Unicode. Instead convert to unicode after it's loaded. If loaded and saved as unicode, and then reloaded, there are encoding errors
  praat_table <- read.csv(praat_table_path, header=TRUE, encoding="")
  
  #praat_table$segnum_ANN_ID <- 1:nrow(praat_table)
  #praat_table$flex_word_tier_ID <- 1:nrow(praat_table)
  #praat_table$MC_graid <- 1:nrow(praat_table)
  #praat_table$MC_refind <- 1:nrow(praat_table)
  #praat_table$MC_isnref <- 1:nrow(praat_table)
  #praat_table$MC_utterance_graid <- 1:nrow(praat_table)
  #praat_table$MC_utterance_refind <- 1:nrow(praat_table)
  #praat_table$MC_utterance_isnref <- 1:nrow(praat_table)
  #praat_table$MC_segnum_ANN_ID <- 1:nrow(praat_table)
  #praat_table$MC_word_tier_ID <- 1:nrow(praat_table)
  #praat_table$phone_position <- 1:nrow(praat_table)

  #set tracking variables for multiple words in a segment
  phone_count <- 0
  word_count <- 1
  MC_word_count <- 1
  MC_phone_count <- 0 #this isn't counting properly and needs work
  save_variable <- 0
}

for(index in 1:(nrow(praat_table))) {
  #index <- 1
  index <- index + index_offset
  if (index > (nrow(praat_table))) {
    index <- (nrow(praat_table))
  }
  ######Check if file is already loaded or differs from currently load file, and if not load it and clean up values#####
  if(!exists("eaf_file")){
    eaf_file <- full_fls[grepl(praat_table$corpus[index], full_fls)]
    eaf <- efileAnnotations(eaf_file)
    #remove long and short vowel symbols, make lowercase
    eaf$VALUE <- enc2native(eaf$VALUE)
    eaf$VALUE <- gsub("<U\\+0306>", "", eaf$VALUE)
    eaf$VALUE <- gsub("<U\\+02D0>", "", eaf$VALUE)
    eaf$VALUE <- gsub("~", "", eaf$VALUE)
    eaf$VALUE <- gsub("<U\\+0283>", "s", eaf$VALUE)
    Encoding(eaf$VALUE) <- "UTF-8"
    eaf$VALUE <- stri_unescape_unicode(gsub("<U\\+(....)>", "\\\\u\\1", eaf$VALUE))
    #to lower must be after unicode escape conversion
    eaf$VALUE <- tolower(eaf$VALUE)
    #generalized attempt to fix writing system tier name issues
    for(tier_entry in 1:nrow(eaf[grepl(flex_morph_tier_name, eaf$TIER_ID),])) {
      annid <- eaf$ANNOTATION_ID[grepl(flex_morph_tier_name, eaf$TIER_ID)][tier_entry]
      if(is.na(eaf$VALUE[grepl(flex_morph_tier_name, eaf$TIER_ID)][tier_entry]) | eaf$VALUE[grepl(flex_morph_tier_name, eaf$TIER_ID)][tier_entry]=="" & length(eaf$VALUE[grepl(flex_morph_tier_name, eaf$TIER_ID) & eaf$ANNOTATION_REF == annid]) != 0) {
        eaf$VALUE[grepl(flex_morph_tier_name, eaf$TIER_ID)][tier_entry] <- eaf$VALUE[grepl(flex_morph_tier_name, eaf$TIER_ID) & eaf$ANNOTATION_REF == annid]
      }
    }
    for(tier_entry in 1:nrow(eaf[grepl(flex_word_tier_name, eaf$TIER_ID),])) {
      annid <- eaf$ANNOTATION_ID[grepl(flex_word_tier_name, eaf$TIER_ID)][tier_entry]
      if(is.na(eaf$VALUE[grepl(flex_word_tier_name, eaf$TIER_ID)][tier_entry]) | eaf$VALUE[grepl(flex_word_tier_name, eaf$TIER_ID)][tier_entry]=="" & length(eaf$VALUE[grepl(flex_word_tier_name, eaf$TIER_ID) & eaf$ANNOTATION_REF == annid]) != 0) {
        eaf$VALUE[grepl(flex_word_tier_name, eaf$TIER_ID)][tier_entry] <- eaf$VALUE[grepl(flex_word_tier_name, eaf$TIER_ID) & eaf$ANNOTATION_REF == annid]
      }
    }
  }
  #if the index has an eaf file different from the last file, reload the file
  if(!grepl(praat_table$corpus[index], eaf_file)){
    eaf_file <- full_fls[grepl(praat_table$corpus[index], full_fls)]
    eaf <- efileAnnotations(eaf_file)
    #remove long and short vowel symbols, make lowercase
    eaf$VALUE <- enc2native(eaf$VALUE)
    eaf$VALUE <- gsub("<U\\+0306>", "", eaf$VALUE)
    eaf$VALUE <- gsub("<U\\+02D0>", "", eaf$VALUE)
    eaf$VALUE <- gsub("~", "", eaf$VALUE)
    eaf$VALUE <- gsub("<U\\+0283>", "s", eaf$VALUE)
    Encoding(eaf$VALUE) <- "UTF-8"
    eaf$VALUE <- stri_unescape_unicode(gsub("<U\\+(....)>", "\\\\u\\1", eaf$VALUE))
    #to lower must be after unicode escape conversion
    eaf$VALUE <- tolower(eaf$VALUE)
    #generalized attempt to fix writing system tier name issues
    for(tier_entry in 1:nrow(eaf[grepl(flex_morph_tier_name, eaf$TIER_ID),])) {
      annid <- eaf$ANNOTATION_ID[grepl(flex_morph_tier_name, eaf$TIER_ID)][tier_entry]
      if(is.na(eaf$VALUE[grepl(flex_morph_tier_name, eaf$TIER_ID)][tier_entry]) | eaf$VALUE[grepl(flex_morph_tier_name, eaf$TIER_ID)][tier_entry]=="" & length(eaf$VALUE[grepl(flex_morph_tier_name, eaf$TIER_ID) & eaf$ANNOTATION_REF == annid]) != 0) {
        eaf$VALUE[grepl(flex_morph_tier_name, eaf$TIER_ID)][tier_entry] <- eaf$VALUE[grepl(flex_morph_tier_name, eaf$TIER_ID) & eaf$ANNOTATION_REF == annid]
      }
    }
    for(tier_entry in 1:nrow(eaf[grepl(flex_word_tier_name, eaf$TIER_ID),])) {
      annid <- eaf$ANNOTATION_ID[grepl(flex_word_tier_name, eaf$TIER_ID)][tier_entry]
      if(is.na(eaf$VALUE[grepl(flex_word_tier_name, eaf$TIER_ID)][tier_entry]) | eaf$VALUE[grepl(flex_word_tier_name, eaf$TIER_ID)][tier_entry]=="" & length(eaf$VALUE[grepl(flex_word_tier_name, eaf$TIER_ID) & eaf$ANNOTATION_REF == annid]) != 0) {
        eaf$VALUE[grepl(flex_word_tier_name, eaf$TIER_ID)][tier_entry] <- eaf$VALUE[grepl(flex_word_tier_name, eaf$TIER_ID) & eaf$ANNOTATION_REF == annid]
      }
    }
  }
  praat_t0 <- praat_table$onset[index]*1000
  praat_t1 <- praat_table$offset[index]*1000
  
  #####load word and phone from praat_table and convert encoding#####
  #praat_table is not loaded as unicode, so it must be converted to unicode to match eaf data
  word <- praat_table$word[index]
  Encoding(word) <- "UTF-8"
  word <- enc2native(word)
  #remove nasal, long, and short vowel symbols
  word <- gsub("<U\\+0306>", "", enc2native(word))
  word <- gsub("<U\\+02D0>", "", enc2native(word))
  word <- gsub("~", "", enc2native(word))
  word <- enc2utf8(word)
  word <- stri_unescape_unicode(gsub("<U\\+(....)>", "\\\\u\\1", word))
  phone <- praat_table$segment[index]
  Encoding(phone) <- "UTF-8"
  phone <- enc2utf8(phone)
  
  #####added +- 30ms because of problem with ail...505 segment ~212, this shouldn't be needed and needs to be investigated
  segnum_ANN_ID <- eaf$ANNOTATION_ID[grepl(segnum_name, eaf$TIER_ID) & round(eaf$t0) >= round(praat_table$sentenceOnset[index]*1000)-30 & round(eaf$t1) <= round(praat_table$sentenceOffset[index]*1000)+30]
      
  #create next segnum_ANN_ID value to track when a segment is finished
  next_praat_t0 <- praat_table$onset[index+1]*1000
  next_praat_t1 <- praat_table$offset[index+1]*1000
  next_segnum_ANN_ID <- eaf$ANNOTATION_ID[grepl(segnum_name, eaf$TIER_ID) & eaf$t0 <= next_praat_t0 & eaf$t1 >= next_praat_t1]
  if (is.na(next_segnum_ANN_ID[1])) {
    next_segnum_ANN_ID<- "NA"
  }
  print(paste("File: ", eaf_file, "Index:", index, "word: ", word, "Phone: ", phone))
  if (length(next_segnum_ANN_ID)==0) {
    next_segnum_ANN_ID <- "no next_segnum_ANN_ID"
  }
  ######test printing for "More than one segnum_ANN_ID#####
  #if(length(segnum_ANN_ID) > 1) {
    #test printing
    #print(paste("More than one segnum_ANN_ID found:", segnum_ANN_ID))
  #}Test printing
  ######test for multiple segments#####
  #for instances where there are more than one segment matching the timing of the phone, search to find which segment has a child tier containing the  word and save the ANNOTATION_ID and ANNOTATION value
  #set segnum_value, segnum_ANN_ID, flex_word_tier_ID
  flex_word_tier_ID <- "NONE"
  for(ANN_ID in segnum_ANN_ID) {
    if(length(eaf$VALUE[grepl(flex_word_tier_name, eaf$TIER_ID,ignore.case=TRUE) & grepl(word, eaf$VALUE, ignore.case=TRUE) & grepl(ANN_ID, eaf$ANNOTATION_REF)])!=0) {
      segnum_value <- eaf$VALUE[grepl(segnum_name, eaf$TIER_ID) & ANN_ID == eaf$ANNOTATION_ID]
      eaf$VALUE[eaf$ANNOTATION_REF[!is.na(eaf$ANNOTATION_REF)]==segnum_ANN_ID]
      #find flex_word annotation ID, subset for flex_word tier, value should equal the value of the word in the praat table, and the annotation ref should equal the annotation ID of the segnum tier
      #This needs to be set up as a for loop to catch multiple instances of the same word in a segment
      segnum_ANN_ID <- ANN_ID
      flex_word_tier_ID <- eaf$ANNOTATION_ID[grepl(flex_word_tier_name, eaf$TIER_ID) & word == eaf$VALUE & ANN_ID == eaf$ANNOTATION_REF]
      #if the word can't find a match, try matching for a less exact match, ie an inflected form
      if (length(flex_word_tier_ID) < 1) {
        word <- eaf$VALUE[grepl(flex_word_tier_name, eaf$TIER_ID) & grepl(word, eaf$VALUE, ignore.case=TRUE) & ANN_ID == eaf$ANNOTATION_REF][1]
        eaf[grepl(flex_word_tier_name, eaf$TIER_ID) & word == eaf$VALUE & ANN_ID == eaf$ANNOTATION_REF,]
        flex_word_tier_ID <- eaf$ANNOTATION_ID[grepl(flex_word_tier_name, eaf$TIER_ID) & word == eaf$VALUE & grepl(ANN_ID, eaf$ANNOTATION_REF)]
      }
    }
  }
    #find flex_word annotation ID, subset for flex_word tier, value should equal the value of the word in the praat table, and the annotation ref should equal the annotation ID of the segnum tier
    #This needs to be set up as a for loop to catch multiple instances of the same word in a segment_number

  ######if the word appears more than once in the segment, determine which one it is.#####
      #test printing
  if(length(flex_word_tier_ID) > 1) {
    flex_word_tier_IDs <- flex_word_tier_ID
    word_count_variable <- paste(gsub("-", "", flex_word_tier_ID[1]))
    if(!exists(word_count_variable)) {
      assign(word_count_variable, word_count)
    }
    #test printing
    #print(paste("More than one flex_word_tier_ID found:", word, "word count: ", eval(parse(text = word_count_variable))))
    #default to the first phone and first instance of the word
    phone_count <- phone_count+1
    #print(paste("phone_count: ", phone_count, ", phone: ", phone))
    #once the first instance is complete, add one to the index to move on to the next instance of the word
    flex_word_tier_ID <- flex_word_tier_IDs[eval(parse(text = word_count_variable))]
    #test printing
    #print(paste("flex_word_tier_ID", flex_word_tier_ID))
    #if this is the last phone of the word, increase the word_count variable by 1
    if(phone_count==nchar(gsub("ː", "", word))) {
      #test printing
      last <- praat_table$segment[index]
      Encoding(last) <- "UTF-8"
      last <- enc2utf8(last)
      #print(paste("last letter", last))
      assign(word_count_variable, eval(parse(text = word_count_variable))+1)
      #test printing
      #print(paste("new word_count", eval(parse(text = word_count_variable))))
      phone_count <- 0
      #test printing
      #print(paste("new phone_count", phone_count))
    }
    #after last phone of last word, reset variables
    if(eval(parse(text = word_count_variable))==length(flex_word_tier_IDs) & phone_count==nchar(sub("ː", "", word))) {
      assign(word_count_variable, 1)
      phone_count <- 0
      word_count <- 1
      #print(paste("last phone of last word", "phone: ", phone,"word: ", word))
      #print(paste("phone_count reset", "word_count reset"))
    }
  }
  #find flex_word annotation ID, subset for flex_word tier, value should equal the value of the word in the praat table, and the annotation ref should equal the annotation ID of the segnum tier
  flex_morph_tier_IDs <- eaf$ANNOTATION_ID[grepl(flex_morph_tier_name, eaf$TIER_ID) & grepl(flex_word_tier_ID, eaf$ANNOTATION_REF)]
  #combine multiple morphs into single entry seperated by "+"
  segmented_morphemes <- paste0(eaf$VALUE[grepl(flex_morph_tier_name, eaf$TIER_ID) & grepl(flex_word_tier_ID, eaf$ANNOTATION_REF) & eaf$VALUE != ""], collapse = "+")
  #use flex_word_tier_ID to find component morphemes, POS's, and glosses, combine multiple values into single string seperated by "+", since there may be multiple morphemes, there needs to be a for loop
  POS_list <- NULL
  gloss_list <- NULL
  for(flex_morph_tier_ID in flex_morph_tier_IDs) {
    POS <- eaf$VALUE[grepl(word_class_name, eaf$TIER_ID) & flex_morph_tier_ID == eaf$ANNOTATION_REF]
    gloss <- eaf$VALUE[grepl(flex_gloss_tier_name, eaf$TIER_ID) & flex_morph_tier_ID == eaf$ANNOTATION_REF]
    #test for NA's and don't add NAs to the list
    if(length(POS)>0){
      if(!is.na(POS) & POS != "NA") {
        POS_list <- append(POS_list, POS)
      }
    }
    if(length(gloss)>0){
      if(!is.na(gloss) & gloss != "NA") {
        gloss_list <- append(gloss_list, gloss)
      }
    }
  }
  #Combine multiple POSs into single entry seperated by "+"
  word_POS <- paste0(POS_list, collapse = "+")
  #Combine multiple glosses into single entry seperated by "+"
  word_gloss <- paste0(gloss_list, collapse = "+")
  
  ######write values to praat_table######
  if (length(segmented_morphemes) > 0) {
    if (segmented_morphemes != "" & !is.na(segmented_morphemes) & segmented_morphemes != "NA" ) {
      praat_table$segmented_morphemes[index] <- segmented_morphemes
    }
  }
  if (length(word_POS) > 0) {
    if (word_POS != "" & !is.na(word_POS) & word_POS != "NA" ) {
      praat_table$word_POS[index] <- word_POS
    }
  }
  if (length(word_gloss) > 0) {
    if (word_gloss != "" & !is.na(word_gloss) & word_gloss != "NA" ) {
      praat_table$word_gloss[index] <- word_gloss
    }
  }
  if (length(segnum_value) > 0) {
    if (segnum_value != "" & !is.na(segnum_value) & segnum_value != "NA" ) {
      praat_table$segnum_value[index] <- segnum_value
    }
  }
  if (length(segnum_ANN_ID) > 0) {
    if (segnum_ANN_ID != "" & !is.na(segnum_ANN_ID) & segnum_ANN_ID != "NA" ) {
      praat_table$segnum_ANN_ID[index] <- segnum_ANN_ID
    }
  }
  if (length(flex_word_tier_ID) > 0) {
    if (flex_word_tier_ID != "" & !is.na(flex_word_tier_ID) & flex_word_tier_ID != "NA" ) {
      praat_table$flex_word_tier_ID[index] <- flex_word_tier_ID
    }
  }
  if (length(flex_word_tier_ID) > 0) {
    if (flex_word_tier_ID != "" & !is.na(flex_word_tier_ID) & flex_word_tier_ID != "NA" ) {
      utterance_table <- praat_table[praat_table$segnum_value==praat_table$segnum_value[index] & praat_table$corpus==praat_table$corpus[index],]
      praat_table$phone_position[index] <- match(praat_table$tokenuniqueid[index], utterance_table$tokenuniqueid)/nrow(utterance_table)
    }
  }
  


  #find and write multi-CAST values
  
  ######find and write multi-CAST values######
  #Check if MC_eaf file is already loaded, and if not load it and clean up values
  if(!exists("MC_eaf_file") & sum(grepl(praat_table$corpus[index], MC_files))>0) {
    MC_eaf_file <- MC_files[grepl(praat_table$corpus[index], MC_files)]
    MC_eaf <- efileAnnotations(MC_eaf_file)
    #remove long and short vowel symbols, make lowercase, remove hyphens and clitic boundaries
    MC_eaf$VALUE <- enc2native(MC_eaf$VALUE)
    MC_eaf$VALUE <- gsub("<U\\+0306>", "", MC_eaf$VALUE)
    MC_eaf$VALUE <- gsub("<U\\+02D0>", "", MC_eaf$VALUE)
    MC_eaf$VALUE <- gsub("~", "", MC_eaf$VALUE)
    MC_eaf$VALUE <- gsub("-", "", MC_eaf$VALUE)
    MC_eaf$VALUE <- gsub("=", "", MC_eaf$VALUE)
    MC_eaf$VALUE <- gsub("<U\\+0283>", "s", MC_eaf$VALUE)
    Encoding(MC_eaf$VALUE) <- "UTF-8"
    MC_eaf$VALUE <- stri_unescape_unicode(gsub("<U\\+(....)>", "\\\\u\\1", MC_eaf$VALUE))
    #to lower must be after unicode escape conversion
    MC_eaf$VALUE <- tolower(MC_eaf$VALUE)
  }
  #if the index has an MC eaf file different from the last file, reload the file
  if(exists("MC_eaf_file") & !grepl(praat_table$corpus[index], MC_eaf_file)) {
    if(!grepl(praat_table$corpus[index], MC_eaf_file) & sum(grepl(praat_table$corpus[index], MC_files))>0){
      MC_eaf_file <- MC_files[grepl(praat_table$corpus[index], MC_files)]
      MC_eaf <- efileAnnotations(MC_eaf_file)
      #remove long and short vowel symbols, make lowercase, remove hyphens and clitic boundaries
      MC_eaf$VALUE <- enc2native(MC_eaf$VALUE)
      MC_eaf$VALUE <- gsub("<U\\+0306>", "", MC_eaf$VALUE)
      MC_eaf$VALUE <- gsub("<U\\+02D0>", "", MC_eaf$VALUE)
      MC_eaf$VALUE <- gsub("~", "", MC_eaf$VALUE)
      MC_eaf$VALUE <- gsub("-", "", MC_eaf$VALUE)
      MC_eaf$VALUE <- gsub("=", "", MC_eaf$VALUE)
      MC_eaf$VALUE <- gsub("<U\\+0283>", "s", MC_eaf$VALUE)
      Encoding(MC_eaf$VALUE) <- "UTF-8"
      MC_eaf$VALUE <- stri_unescape_unicode(gsub("<U\\+(....)>", "\\\\u\\1", MC_eaf$VALUE))
      #to lower must be after unicode escape conversion
      MC_eaf$VALUE <- tolower(MC_eaf$VALUE)
    }
  }
  #If the current file has a MC eaf availaible, find and write the values
  if(exists("MC_eaf_file") & sum(grepl(praat_table$corpus[index], MC_files))>0) {
    if (sum(grepl(praat_table$corpus[index], MC_files)) > 0 & length(MC_eaf$ANNOTATION_ID[grepl(MC_segnum_name, MC_eaf$TIER_ID) & MC_eaf$t0 == praat_table$sentenceOnset[index]*1000 & MC_eaf$t1 == praat_table$sentenceOffset[index]*1000]) != 0) {
      MC_segnum_ANN_ID <- MC_eaf$ANNOTATION_ID[grepl(MC_segnum_name, MC_eaf$TIER_ID) & MC_eaf$t0 == praat_table$sentenceOnset[index]*1000 & MC_eaf$t1 == praat_table$sentenceOffset[index]*1000]
    }
    for(MC_ANN_ID in MC_segnum_ANN_ID) {
      #test
      #MC_ANN_ID <- MC_segnum_ANN_ID
      if(length(MC_eaf$VALUE[grepl(MC_utterance_name, MC_eaf$TIER_ID, ignore.case=TRUE) & grepl(word, MC_eaf$VALUE, ignore.case=TRUE) & MC_ANN_ID == MC_eaf$ANNOTATION_REF])!=0) {
        MC_segnum_value <- MC_eaf$VALUE[grepl(MC_segnum_name, MC_eaf$TIER_ID) & MC_ANN_ID == MC_eaf$ANNOTATION_ID]
        #find flex_word annotation ID, subset for flex_word tier, value should equal the value of the word in the praat table, and the annotation ref should equal the annotation ID of the segnum tier
        #This needs to be set up as a for loop to catch multiple instances of the same word in a segment
        MC_segnum_ANN_ID <- MC_ANN_ID
        MC_utterance_ID <- MC_eaf$ANNOTATION_ID[MC_utterance_name == MC_eaf$TIER_ID & MC_segnum_ANN_ID == MC_eaf$ANNOTATION_REF]
        MC_word_tier_ID <- MC_eaf$ANNOTATION_ID[grepl(MC_word_tier_name, MC_eaf$TIER_ID) & MC_utterance_ID == MC_eaf$ANNOTATION_REF & grepl(word, MC_eaf$VALUE, ignore.case=TRUE)] #"noːɛːmɛː" 
        #if the word can't find a match, try matching for a less exact match, ie an inflected form
        if (length(MC_word_tier_ID) < 1) {
          word <- MC_eaf$VALUE[grepl(MC_word_tier_name, MC_eaf$TIER_ID) & grepl(word, MC_eaf$VALUE, ignore.case=TRUE) & grepl(MC_ANN_ID, MC_eaf$ANNOTATION_REF)][1]
          MC_word_tier_ID <- MC_eaf$ANNOTATION_ID[grepl(MC_word_tier_name, MC_eaf$TIER_ID) & word == MC_eaf$VALUE & grepl(MC_ANN_ID, MC_eaf$ANNOTATION_REF)]
        }
      }
    }
    
    #####find MC_word annotation ID#####
    #subset for flex_word tier, value should equal the value of the word in the praat table, and the annotation ref should equal the annotation ID of the segnum tier
    #This needs to be set up as a for loop to catch multiple instances of the same word in a segment_number
    #find flex_word annotation ID, subset for flex_word tier, value should equal the value of the word in the praat table, and the annotation ref should equal the annotation ID of the segnum tier
    #This needs to be set up as a for loop to catch multiple instances of the same word in a segment_number
    
    #if the word appears more than once in the segment, determine which one it is.
    if(length(MC_word_tier_ID[!is.na(MC_word_tier_ID)]) > 0) {
      MC_word_tier_IDs <- MC_word_tier_ID
      MC_word_count_variable <- paste0("MC", gsub("-", "", MC_word_tier_ID[1]))
      if(!exists(MC_word_count_variable)) {
        assign(MC_word_count_variable, MC_word_count)
      }
      #test printing
      #print(paste("More than one MC_word_tier_ID found:", word, "word count: ", eval(parse(text = MC_word_count_variable))))
      #default to the first phone and first instance of the word
      MC_phone_count <- MC_phone_count+1
      #print(paste("MC_phone_count: ", MC_phone_count, ", phone: ", phone))
      #once the first instance is complete, add one to the index to move on to the next instance of the word
      MC_word_tier_ID <- MC_word_tier_IDs[eval(parse(text = MC_word_count_variable))]
      #test printing
      #print(paste("MC_word_tier_ID", MC_word_tier_ID))
      #if this is the last phone of the word, increase the MC_word_count variable by 1
      if(MC_phone_count==nchar(word)) {
        #test printing
        last <- praat_table$segment[index]
        Encoding(last) <- "UTF-8"
        last <- enc2utf8(last)
        #print(paste("last letter", last))
        assign(MC_word_count_variable, eval(parse(text = MC_word_count_variable))+1)
        #test printing
        #print(paste("new MC_word_count", eval(parse(text = MC_word_count_variable))))
        MC_phone_count <- 0
        #test printing
        #print(paste("new MC_phone_count", MC_phone_count))
      }
      #after last phone of last word, reset variables
      if(eval(parse(text = MC_word_count_variable))==length(MC_word_tier_IDs) & MC_phone_count==nchar(sub("ː", "", word))) {
        assign(MC_word_count_variable, 1)
        MC_phone_count <- 0
        MC_word_count <- 1
        #print(paste("last phone of last word", "phone: ", phone,"word: ", word))
        #print(paste("MC_phone_count reset", "MC_word_count reset"))
      }
    }
    
    ######find MC_Gloss annotation ID######
    if(length(MC_word_tier_ID[!is.na(MC_word_tier_ID)]) > 0) {
      #find word annotation ID, subset for word tier, value should equal the value of the word in the praat table, and the annotation ref should equal the annotation ID of the segnum tier
      MC_gloss_tier_ID <- MC_eaf$ANNOTATION_ID[grepl(MC_gloss_tier_name, MC_eaf$TIER_ID) & MC_word_tier_ID == MC_eaf$ANNOTATION_REF]
      #use word_tier_ID to find component morphemes, POS's, and glosses, combine multiple values into single string seperated by "+", since there may be multiple morphemes, there needs to be a for loop
      MC_gloss <- MC_eaf$VALUE[grepl(MC_gloss_tier_name, MC_eaf$TIER_ID) & MC_word_tier_ID == MC_eaf$ANNOTATION_REF]
      #test for NA's and don't add NAs to the list
      
      ###########find MC_graid annotation ID######
      MC_graid_tier_ID <- MC_eaf$ANNOTATION_ID[grepl(MC_graid_tier_name, MC_eaf$TIER_ID) & MC_word_tier_ID == MC_eaf$ANNOTATION_REF]
      #use word_tier_ID to find component morphemes, POS's, and glosses, combine multiple values into single string seperated by "+", since there may be multiple morphemes, there needs to be a for loop
      MC_graid <- MC_eaf$VALUE[grepl(MC_graid_tier_name, MC_eaf$TIER_ID) & MC_word_tier_ID == MC_eaf$ANNOTATION_REF]
      
      ###########find MC_refind annotation ID######
      MC_refind_tier_ID <- MC_eaf$ANNOTATION_ID[grepl(MC_refind_tier_name, MC_eaf$TIER_ID) & MC_graid_tier_ID == MC_eaf$ANNOTATION_REF]
      #use word_tier_ID to find component morphemes, POS's, and glosses, combine multiple values into single string seperated by "+", since there may be multiple morphemes, there needs to be a for loop
      MC_refind <- MC_eaf$VALUE[grepl(MC_refind_tier_name, MC_eaf$TIER_ID) & MC_graid_tier_ID == MC_eaf$ANNOTATION_REF]
      
      ###########find MC_isnref annotation ID######
      MC_isnref_tier_ID <- MC_eaf$ANNOTATION_ID[grepl(MC_isnref_tier_name, MC_eaf$TIER_ID) & MC_refind_tier_ID == MC_eaf$ANNOTATION_REF]
      #use word_tier_ID to find component morphemes, POS's, and glosses, combine multiple values into single string seperated by "+", since there may be multiple morphemes, there needs to be a for loop
      MC_isnref <- MC_eaf$VALUE[grepl(MC_isnref_tier_name, MC_eaf$TIER_ID) & MC_refind_tier_ID == MC_eaf$ANNOTATION_REF]
      
      ######MC_grammatical words########
      MC_utterance_list <- MC_eaf$VALUE[grepl(MC_word_tier_name, MC_eaf$TIER_ID) & MC_utterance_ID == MC_eaf$ANNOTATION_REF]
      MC_utterance <- paste0(MC_utterance_list, collapse = "+")
      
      ######MC_utterance_graid#########
      MC_utterance_word_IDs <- MC_eaf$ANNOTATION_ID[grepl(MC_word_tier_name, MC_eaf$TIER_ID) & MC_utterance_ID == MC_eaf$ANNOTATION_REF]
      MC_utterance_graid_list <- NULL
      MC_utterance_graid_IDs <- NULL
      for (ID in MC_utterance_word_IDs) {
        MC_utterance_graid <- MC_eaf$VALUE[grepl(MC_graid_tier_name, MC_eaf$TIER_ID) & ID == MC_eaf$ANNOTATION_REF]
        MC_utterance_graid_ID <- MC_eaf$ANNOTATION_ID[grepl(MC_graid_tier_name, MC_eaf$TIER_ID) & ID == MC_eaf$ANNOTATION_REF]
        if(length(MC_utterance_graid) > 0) {
          if(!is.na(MC_utterance_graid) & MC_utterance_graid != "NA") {
            MC_utterance_graid_list <- append(MC_utterance_graid_list, MC_utterance_graid)
            MC_utterance_graid_IDs <- append(MC_utterance_graid_IDs, MC_utterance_graid_ID)
          }  
        }  
      }
      MC_utterance_graid <- paste(MC_utterance_graid_list, collapse = "+")
      
      ######MC_utterance_refind#######
      MC_utterance_refind_list <- NULL
      MC_utterance_refind_IDs <- NULL
      for (ID in MC_utterance_graid_IDs) {
        MC_utterance_refind <- MC_eaf$VALUE[grepl(MC_refind_tier_name, MC_eaf$TIER_ID) & ID == MC_eaf$ANNOTATION_REF]
        MC_utterance_refind_ID <- MC_eaf$ANNOTATION_ID[grepl(MC_refind_tier_name, MC_eaf$TIER_ID) & ID == MC_eaf$ANNOTATION_REF]
        if(length(MC_utterance_refind) > 0) {
          if(!is.na(MC_utterance_refind) & MC_utterance_refind != "NA") {
            MC_utterance_refind_list <- append(MC_utterance_refind_list, MC_utterance_refind)
            MC_utterance_refind_IDs <- append(MC_utterance_refind_IDs, MC_utterance_refind_ID)
          }
        }
      }
      MC_utterance_refind <- paste(MC_utterance_refind_list, collapse = "+")
      
      ######MC_utterance_isnref########
      MC_utterance_isnref_list <- NULL
      MC_utterance_isnref_IDs <- NULL
      for (ID in MC_utterance_refind_IDs) {
        #ID <- MC_utterance_graid_IDs[4]
        MC_utterance_isnref <- MC_eaf$VALUE[grepl(MC_isnref_tier_name, MC_eaf$TIER_ID) & ID == MC_eaf$ANNOTATION_REF]
        MC_utterance_isnref_ID <- MC_eaf$ANNOTATION_ID[grepl(MC_isnref_tier_name, MC_eaf$TIER_ID) & ID == MC_eaf$ANNOTATION_REF]
        if(length(MC_utterance_isnref) > 0) {
          if(!is.na(MC_utterance_isnref) & MC_utterance_isnref != "NA") {
            MC_utterance_isnref_list <- append(MC_utterance_isnref_list, MC_utterance_isnref)
            MC_utterance_isnref_IDs <- append(MC_utterance_isnref_IDs, MC_utterance_isnref_ID)
          }
        }
      }
      MC_utterance_isnref <- paste(MC_utterance_isnref_list, collapse = "+")  
      
      
      #####Write MC values to Praat table#####
      if (length(MC_graid) > 0) {
        if (MC_graid != "" & !is.na(MC_graid) & MC_graid != "NA" ) {
          praat_table$MC_graid[index] <- MC_graid
        }
      }
      if (length(MC_refind) > 0) {
        if (MC_refind != "" & !is.na(MC_refind) & MC_refind != "NA" ) {
          praat_table$MC_refind[index] <- MC_refind
        }
      }
      if (length(MC_isnref) > 0) {
        if (MC_isnref != "" & !is.na(MC_isnref) & MC_isnref != "NA" ) {
          praat_table$MC_isnref[index] <- MC_isnref
        }
      }
      if (length(MC_utterance_graid) > 0) {
        if (MC_utterance_graid != "" & !is.na(MC_utterance_graid) & MC_utterance_graid != "NA" ) {
          praat_table$MC_utterance_graid[index] <- MC_utterance_graid
        }
      }
      if (length(MC_utterance_refind) > 0) {
        if (MC_utterance_refind != "" & !is.na(MC_utterance_refind) & MC_utterance_refind != "NA" ) {
          praat_table$MC_utterance_refind[index] <- MC_utterance_refind
        }
      }
      if (length(MC_utterance_isnref) > 0) {
        if (MC_utterance_isnref != "" & !is.na(MC_utterance_isnref) & MC_utterance_isnref != "NA" ) {
          praat_table$MC_utterance_isnref[index] <- MC_utterance_isnref
        }
      }
      if (length(MC_segnum_ANN_ID) > 0) {
        if (MC_segnum_ANN_ID != "" & !is.na(MC_segnum_ANN_ID) & MC_segnum_ANN_ID != "NA" ) {
          praat_table$MC_segnum_ANN_ID[index] <- MC_segnum_ANN_ID
        }
      }
      if (length(MC_word_tier_ID) > 0) {
        if (MC_word_tier_ID != "" & !is.na(MC_word_tier_ID) & MC_word_tier_ID != "NA" ) {
          praat_table$MC_word_tier_ID[index] <- MC_word_tier_ID
        }
      }
      
      #####Test printing result######
      print(paste("MC_graid: ", MC_graid, "MC_refind: ", MC_refind, "MC_isnref: ", MC_isnref))
      #####
    }
  }  
  ######after every hudredth segment is finished, write the results to a backup file#####
  print(paste("word: ", word, "segmented_morphemes: ", segmented_morphemes, "word_POS: ", word_POS, "word_gloss: ",  word_gloss, "segment_number: ", segnum_value))
  if(length(segnum_ANN_ID) > 0 & length(next_segnum_ANN_ID[1]) > 0){
    if((segnum_ANN_ID[1] != next_segnum_ANN_ID[1] & save_variable %% 100 == 0)|index == nrow(praat_table)) {
      #modify to save after a segment with segment and index in name
      save_variable <- save_variable+1
      output <- getwd()
      write.csv(praat_table, paste(output, languageFolder, paste("praat_table_updated_", "last_segnum_", segnum_value, "_index_", index, ".csv", sep=""), sep="/"), row.names = FALSE, fileEncoding = "")
      print(paste("praat_table_updated, ", "last_segnum :", segnum_value, sep = ""))
    }
  }
  ######test printing for missing values######
  #test printing
  #if (word == "" | segmented_morphemes == "" | word_POS == "" | word_gloss == "" | segnum_value == "") {
  #print(paste("MISSING VALUES in File: ", eaf_file, "Index:", index))
    #}
  #####
}

